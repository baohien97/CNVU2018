...
class RoutingTable(object):
    ...
    def construct_routing_table(self, vectors):
        ...
        for (line, table) in vectors.items():
            for dest in table: # store optimal values in tuple form (delay, line)
...
def main():
    ...
    # table that updates delay vectors of all neighbours
    neighbour_vectors = {}
    for packet in packets:
        if packet[0] == 'R':
            print('THANK YOU')
            source_r_address, delay_to_cur, entries = RoutingTable.read_r_packets(packet)
            packet_table = {} # vector of each neighbour
