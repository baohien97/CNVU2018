...
def backward_learning():
    ...
    for frame in frame_list:
        source_port = int(frame[0])  # port on which frame is received
        source_address = frame[6:10]  # address of current machine
        target_address = frame[2:6]  # address of target machine
        ...
        for (key, val) in mac_dict.items():
            if target_address in val:
                ...
                if key == source_port:
                    ...
                    target_port = str(key)
                    output_frame = frame[2:] + " " + target_port
        if location_seen == False: # location unknown => flooding
            output_frame = frame[2:] + " " + " ".join(str(i) for i in all_ports if i != source_port)
